...
// ---------------------------------------------------------------------------
// Minimal base64 decoder (JWT payload only — no signature verification)
// ---------------------------------------------------------------------------

const _B64_CHARS =
  'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';

/** Decode a standard base64 string to a UTF-8 string, without atob/Buffer. */
...
export class AuthService {
  ...
  /**
   * Return the backend user id (the JWT `sub` claim), decoded from the stored
   * session token. This is the stable identifier the backend uses as the
   * RevenueCat app_user_id. Returns null when there is no valid token.
   *
   * NOTE: this only base64url-decodes the JWT payload for the id — it does NOT
   * verify the signature (the server does that). Never trust it for auth
   * decisions; use it only as an opaque account identifier.
   */
