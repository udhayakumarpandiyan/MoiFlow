...
import { subscriptionService, revenueCatService, authService } from '@common/services';
...
export const EntitlementProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 2. Configure RevenueCat with the backend user id (its app_user_id) so
      //    purchases and webhooks map to this account. Best-effort: if the user
      //    isn't logged in yet or the native module is absent, this no-ops.
      try {
      ...
      // 3. Re-verify entitlement (RevenueCat + backend) in the background.
      //    Never downgrades on a transient failure (returns cached state).
      try {
