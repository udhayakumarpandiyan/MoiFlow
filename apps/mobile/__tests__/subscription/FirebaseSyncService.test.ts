...
import { FirebaseSyncService } from '../../src/common/subscription/FirebaseSyncService';
import type { ISyncQueueRepository } from '../../src/common/repository/interfaces/ISyncQueueRepository';
import type { SyncQueue } from '../../src/common/models/SyncQueue';

// Control what featureGuard reports without loading the DI container.
jest.mock('../../src/common/subscription/featureGuard', () => {
  return {
    __premium: { value: true },
    assertPremiumFeature: jest.fn(() => {
      if (!(jest.requireMock('../../src/common/subscription/featureGuard') as any).__premium.value) {
    ...
    hasPremiumFeature: jest.fn(
      () => (jest.requireMock('../../src/common/subscription/featureGuard') as any).__premium.value,
    ),
  };
});

// eslint-disable-next-line @typescript-eslint/no-var-requires
const guardMock = jest.requireMock('../../src/common/subscription/featureGuard') as any;
